...
public class Load {
	...
	public static void Register(Plugin pl, CommandManager manager) throws IOException, InstantiationException, IllegalAccessException, ClassNotFoundException {
	...
	private static boolean hasListenerAdd(Annotation[] a) {
		for(Annotation b : a) if(b instanceof AddListener) return true;
		return false;
	}
	private static boolean hasListener(Class<?>[] clazz) {
		for(Class<?> c : clazz) if(c == Listener.class) return true;
		return false;
	}
